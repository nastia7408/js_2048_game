...
function updateStatusUI() {
    ...
    document.querySelector(".message-start").classList.toggle("hidden", gameStatus !== "idle");
...
class Game {
    constructor(initialState){
        this.tileCounter = 0;
        this.field = initialState || Array((0, _constantsJs.GRID_SIZE)).fill().map(()=>Array((0, _constantsJs.GRID_SIZE)).fill(null));
        this.score = 0;
        this.gameStatus = (0, _constantsJs.STATUS).IDLE;
    }
    createTile(value) {
        return {
            id: this.tileCounter++,
    ...
    moveLeft() {
        ...
        for(let r = 0; r < (0, _constantsJs.GRID_SIZE); r++){
            const row = this.field[r].filter((cell)=>cell !== null);
            const newRow = Array((0, _constantsJs.GRID_SIZE)).fill(null);
            for(let i = 0; i < row.length; i++)if (i < row.length - 1 && row[i].value === row[i + 1].value) {
    ...
    addRandomTile() {
        const empty = [];
        for(let r = 0; r < (0, _constantsJs.GRID_SIZE); r++){
            for(let c = 0; c < (0, _constantsJs.GRID_SIZE); c++)if (!this.field[r][c]) empty.push({
                r,
                c
            });
        }
    ...
    checkWin() {
        if (this.field.flat().some((tile)=>tile?.value === (0, _constantsJs.WIN_TARGET))) this.gameStatus = (0, _constantsJs.STATUS).WIN;
    }
    checkGameOver() {
        if (this.field.flat().some((tile)=>tile === null)) return;
        for(let r = 0; r < (0, _constantsJs.GRID_SIZE); r++)for(let c = 0; c < (0, _constantsJs.GRID_SIZE); c++){
            const val = this.field[r][c].value;
            if (c < (0, _constantsJs.GRID_SIZE) - 1 && val === this.field[r][c + 1].value) return;
            if (r < (0, _constantsJs.GRID_SIZE) - 1 && val === this.field[r + 1][c].value) return;
        }
        this.gameStatus = (0, _constantsJs.STATUS).LOSE;
    }
    start() {
        this.gameStatus = (0, _constantsJs.STATUS).PLAYING;
        this.addRandomTile();
        this.addRandomTile();
    }
    restart() {
        this.field = Array((0, _constantsJs.GRID_SIZE)).fill().map(()=>Array((0, _constantsJs.GRID_SIZE)).fill(null));
        this.score = 0;
        this.gameStatus = (0, _constantsJs.STATUS).IDLE;
        this.start();
    }
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./constants.js":"e0Zqh"}],"gkKU3":[function(require,module,exports) {
...
},{}],"e0Zqh":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "GRID_SIZE", ()=>GRID_SIZE);
parcelHelpers.export(exports, "WIN_TARGET", ()=>WIN_TARGET);
parcelHelpers.export(exports, "PROBABILITY_4", ()=>PROBABILITY_4);
parcelHelpers.export(exports, "INITIAL_TILES", ()=>INITIAL_TILES);
